...
- Produces: `variantesTelefone(whatsappNumber)` → `string[]` (puro);
  `resolverIdentidade({ contact })` → `Promise<{ nivel:'forte'|'none', origem:'memory'|'phone'|'none', primeiroNome:string|null, contracts:[], client:{id,document}|null, dataNascimento:string|null, contestado:false, nascimentoTentado:false }>` — **sem `name`**: o nome completo nunca sai do resolutor; quem precisa do nome usa `primeiroNome`.
...
- [ ] **Step 2c: A contestação do nome persiste entre turnos** (ruling da
revisão da Task 4). Depois de `esquecer_identificacao`, o vínculo do contato é
limpo — mas `resolverIdentidade` do turno seguinte buscaria o **mesmo
telefone** no SGP e cumprimentaria a mesma pessoa errada de novo.

1. Migração `migrations/1789020000000_add-ai-triage-phone-contested.js`:
   `ALTER TABLE conversations ADD COLUMN IF NOT EXISTS ai_triage_phone_contested BOOLEAN NOT NULL DEFAULT false;`
   (down: DROP COLUMN). Coluna lida só pelas duas funções abaixo — **fora** do
   mapper e das 25 consultas, como `ai_triage_birthdate_attempts`.
2. `conversation.repository.js`: `markPhoneContested(conversationId)` →
   `UPDATE conversations SET ai_triage_phone_contested = true WHERE id = $1`;
   `isPhoneContested(conversationId)` → `SELECT ai_triage_phone_contested FROM conversations WHERE id = $1` → boolean (false se não achar). Teste real no banco.
3. `identity-resolver.js`: `resolverIdentidade({ contact, ignorarTelefone = false })` — com `true`, pula a busca por telefone (memória continua valendo). Teste.
4. `tool-registry.js` `esquecer_identificacao.executar`: chama `markPhoneContested(contexto.conversationId)` (dentro de try/catch com log seguro — a limpeza em memória e do vínculo não pode falhar por isso). Teste: chamada feita.
5. Worker (Step 4): `const ignorarTelefone = await isPhoneContested(conversation.id);` e `resolverIdentidade({ contact, ignorarTelefone })`. Teste: com a coluna marcada, o resolutor recebe `ignorarTelefone: true`.

- [ ] **Step 3: Testes do worker** (`ai-worker.test.js`; mocks novos: `../ai/identity-resolver`, `../channels/channel.repository`, `../queue/outbound-queue`; `findLatestInboundMessageId` já mockado — no perfil de triagem o worker a chama com `{ qualquerTipo: true }`, e um teste deve afirmar isso; o objeto `identidade` devolvido por `runAiTurn` carrega `dataNascimento` e o CPF — o worker **não** o loga nem o persiste):
...
async function handleTriageTurn({ conversation, config, messageId }) {
  ...
  // Flag desligada: só retorna. Concluir aqui abortaria a triagem NUMÉRICA de
  // um canal recém-migrado para IA (ruling da revisão); a triagem IA legítima
  // interrompida é encerrada pelo job de timeout.
  if (!channel || !channel.aiEnabled || !channel.aiTriageEnabled) return;
  // Na triagem, "a mais nova" conta exatamente os tipos que scheduleAiTriage
  // enfileira (text, image, document, audio) — sticker/vídeo/localização não
  // geram turno e por isso não podem tornar o job anterior "ultrapassado".
