...
> - na T7, o Step 5, que ao pé da letra quebrava **toda** linha da lista (apagava `formatMessageTime`, que a linha compacta usa) e apagava o único teste da hora da última mensagem na linha; e o erro do "Carregar mais", sem teste;
...
**[Prova na cópia, 24/09/2026]** Em `ConversationListItem.test.jsx`, os testes da variante não compacta (os que renderizam sem `compact` nem `rail`) saem junto com ela — conferir com `grep -n "compact\|rail" src/components/ConversationListItem.test.jsx`. **Exceto o `describe('hora mostrada na linha')`**. A hora continua na linha compacta: a da última mensagem, por padrão, e a de chegada na fila, por `showArrivalTime`. Ele é o único teste da hora da **última mensagem** na linha. Medido na cópia: sem ele, uma linha que mostrasse a hora de chegada no lugar da última mensagem passava com a suíte inteira verde. (A hora de chegada também é coberta por `QueueList.test.jsx`, "a linha mostra a hora em que o cliente entrou na fila". A prova tinha dito que ele era o único teste de `showArrivalTime`, e isso estava errado.) Ele fica, com `compact` nas duas montagens (a do `renderItem` e a do terceiro teste). Ao pé da letra, a regra apagava 49 testes, entre eles este. O que os outros cobriam e continua valendo (rótulo de mídia sem emoji, Pix sem código) está nos testes da linha compacta da E2.3.
