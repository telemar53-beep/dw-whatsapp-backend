...
function VoiceNote({ url, seed, outbound, avatar }) {
...
function ImageBubble({ url, alt, hasCaption }) {
  ...
  return (
    <>
      <button
        ...
        className={`block overflow-hidden rounded-[6px] focus-visible:outline focus-visible:outline-2 focus-visible:outline-wa-green ${
          hasCaption ? 'mb-1' : ''
        }`}
      >
        <img
...
function DocumentCard({ url, filename, outbound }) {
  ...
  return (
    ...
    >
      ...
      <span className="min-w-0 flex-1">
        <span className="block truncate text-[13.5px] leading-tight text-wa-text">{label}</span>
        <span className="mt-0.5 block text-[11px] uppercase tracking-wide text-wa-meta">{extension}</span>
      </span>
      <span className="shrink-0 text-wa-icon opacity-70">
...
function MessageAttachment({ message, avatar }) {
  const { token } = useAuth();
  ...
  if (message.messageType === 'location') {
    ...
    return (
      <a
        ...
        target="_blank"
        rel="noreferrer"
        className="block w-[min(16rem,60vw)] overflow-hidden rounded-[6px] bg-[#e6e0d8]"
      >
        ...
        <span className="flex items-center justify-between gap-2 px-3 py-2 text-[13px] text-wa-text">
          Ver localização no mapa
          <span className="-rotate-90 text-wa-icon opacity-70">
  ...
  if (message.messageType === 'image') {
    return <ImageBubble url={url} alt={message.mediaFilename || 'Imagem'} hasCaption={Boolean(message.content)} />;
  }
  ...
  if (message.messageType === 'audio') {
    return <VoiceNote url={url} seed={message.id || ''} outbound={outbound} avatar={avatar} />;
  }

  if (message.messageType === 'video') {
    return <video controls src={url} className="max-w-full rounded-[6px]" style={{ maxHeight: 340, minWidth: 200 }} />;
  }

  if (message.messageType === 'document') {
    return <DocumentCard url={url} filename={message.mediaFilename} outbound={outbound} />;
  }
