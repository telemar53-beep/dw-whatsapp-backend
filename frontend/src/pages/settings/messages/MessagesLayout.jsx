    ...
    <ProtectedRoute level="admin" areaLabel="Mensagens e templates">
      <div className="flex min-h-0 flex-1 flex-col">
        <div className="settings-group-header border-b border-white/[0.07] px-4">
          <PageHeader
            crumbs={[{ label: 'Configurações', to: SETTINGS_BASE }, { label: selected?.key === 'boas-vindas' || selected?.key === 'abertura-encerramento' ? 'Atendimento' : 'Mensagens' }]}
            title={<SettingsTitle name={selected?.key}>{selected?.label || 'Mensagens'}</SettingsTitle>}
